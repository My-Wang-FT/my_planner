...
ros::Publisher _all_map_pub, _grid_map_vis_pub;
...
bool _has_map = false, _has_map_vis = false;

sensor_msgs::PointCloud2 globalMap_pcd, map_vis;
...
void VisualMapGenerate()
{
    ...
    pcl::fromROSMsg(globalMap_pcd, cloud);
    ...
    for (int idx = 0; idx < (int)cloud.points.size(); idx++)
    {
        ...
        pt.x = min(max(double(pt.x), _x_l), _x_h);
        pt.y = min(max(double(pt.y), _y_l), _y_h);
