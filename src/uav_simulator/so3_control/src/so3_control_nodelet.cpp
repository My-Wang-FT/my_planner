...
public:
  SO3ControlNodelet()
      : position_cmd_updated_(false), position_cmd_init_(false), des_yaw_(0), des_yaw_dot_(0), current_yaw_(0), enable_motors_(true), // FIXME
...
private:
  ...
  bool position_cmd_updated_, position_cmd_init_, use_hopf;
...
{
  controller_.calculateControl(des_pos_, des_vel_, des_acc_, des_jerk_, des_yaw_,
                               des_yaw_dot_, kx_, kv_, use_hopf);
...
void SO3ControlNodelet::odom_callback(const nav_msgs::Odometry::ConstPtr &odom)
{
  ...
  else if (init_z_ > -9999.0)
  {
    ...
    des_jerk_ = Eigen::Vector3d(0, 0, 0);
    publishSO3Command();
  }
}
...
void SO3ControlNodelet::onInit(void)
{
  ...
  n.param("use_hopf", use_hopf, true);
