...
int main(int argc, char **argv)
{
    ...
    // publish the commanded local position
    Pose_cmd_pub = nh.advertise<quadrotor_msgs::PositionCommand>
        ("/planning/goal_cmd", 10);
    // servo_cmd_pub = nh.advertise<std_msgs::String>
    //     ("servo_command", 10);

    ros::Subscriber Odom_sub = nh.subscribe<nav_msgs::Odometry>
        ("/visual_slam/odom", 10, Odom_cb);
    ...
    while( ros::ok() ){
        ...
        if ( dis < DistanceThreshold ) {
           ...
           // ROS_INFO("[test_goal] Time: %f  dis: %f",ros::Time::now()-last_request,dis);
            if (ros::Time::now()-last_request > ros::Duration(wait_time) ){
                
                currentWaypoint ++;

                if ( currentWaypoint >= TargetPointCount ) { // reachedHeight
                    break;
                }
